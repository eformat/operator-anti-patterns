...
RUN microdnf install -y python3 findutils tar gzip && microdnf clean all
...
WORKDIR /opt/scanner
COPY scan-operator-antipatterns.sh .semgrep-operator-antipatterns.yml SKILL.md ./
COPY evaluations/ evaluations/
COPY testdata/ testdata/
RUN chmod +x scan-operator-antipatterns.sh
